...
mydict = ['LeTien', 'NgocDuc', 'TaiTam', 'TrungNghia']
    ...
    while True:
        ...
        if faces[1] is not None:
            ...
            for idx, face in enumerate(faces[1]):
                ...
                face_feature = face_feature.reshape(1, -1)  # Reshape to a 2D array with 1 row
                test_predict = svc.predict(face_feature)
                result = mydict[test_predict[0]]

                # Store recognized face label and coordinates
                recognized_faces.append({
                    'label': result,
                    'coordinates': face.astype(np.int32)[:-1]  # Exclude the score
                })
            ...
            # Visualize results and recognized labels
            for face_info in recognized_faces:
                label = face_info['label']
                coordinates = face_info['coordinates']
                cv.putText(frame, label, (coordinates[0], coordinates[1] - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
